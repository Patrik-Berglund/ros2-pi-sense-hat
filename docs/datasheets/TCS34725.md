...
##### − Low Power — 2.5-  A Sleep State

##### − 65-  A Wait State with Programmable Wait
...
###  TVs, Mobile Handsets, Tablets, Computers,
...
###### 1001 Klein Road  Suite 300  Plano, TX 75074  (972) 673-
...
##### (ADC) that integrate the photodiode current, data registers, a state machine, and an I^2 C interface. The 3 × 4
...
##### Communication of the TCS3472 data is accomplished over a fast, up to 400 kHz, two-wire I^2 C serial bus. The

##### industry standard I^2 C bus facilitates easy, direct connection to microcontrollers and embedded processors.

##### In addition to the I^2 C bus, the TCS3472 provides a separate interrupt signal output. When interrupts are
...
```
TERMINAL
NAME NO. TYPETYPE DESCRIPTIONDESCRIPTION
...
TCS34721† 0x39 FN−6 I^2 C Vbus = VDD Interface TCS34721FN
TCS34723† 0x39 FN−6 I^2 C Vbus = 1.8 V Interface TCS34723FN
TCS34725 0x29 FN−6 I^2 C Vbus = VDD Interface TCS34725FN
...
```
MIN NOM MAX UNIT
Supply voltage, VDD (TCS34721 & TCS34725) (I^2 C Vbus = VDD) 2.7 3 3.6 V
Supply voltage, VDD (TCS34723 & TCS34727) (I^2 C Vbus = 1.8 V) 2.7 3 3.3 V
...
Active 235 330
IIDDDD Supply currentSupply current Wait state^65 μA
Sleep state — no I^2 C activity 2.5 10
```
```
μA
```
```
V INT SDA output low voltage
```
```
3 mA sink current 0 0.
VOL INT, SDA output low voltage 6 mA sink current 0 0.6 VV
ILEAK Leakage current, SDA, SCL, INT pins −5 5 μA
ILEAK Leakage current, LDR pin −5 5 μA
V SCL SDA input high voltage
```
```
TCS34721 & TCS34725 0.7 VDD
VIH SCL, SDA input high voltage TCS34723 & TCS34727 1.25 VV
```
```
V SCL SDA input low voltage
```
```
TCS34721 & TCS34725 0.3 VDD
VIL SCL, SDA input low voltage TCS34723 & TCS34727 0.54 VV
```
...
PARAMETER CONDITIONSTEST MIN TYP MAX MIN TYP MAX MIN TYP MAX MIN TYP MAX UNITUNIT
λD = 465 nm
Note 2 0% 15% 10% 42% 65% 88% 11.0 13.8 16.
Re Irradianceresponsivity λNote 3D = 525 nm 4% 25% 60% 85% 10% 45% 13.2 16.6 20.
```
```
counts/
μW/
responsivity cm 2
λD = 615 nm
Note 4 80% 110% 0% 14% 5% 24% 15.6 19.5 23.
```
```
cm^2
```
...
### Gain scalinGain scalingg, relative to 1, relative to 1 ×× ggainain 16 × 15 16 16 8 

### settingsetting^16 ×^1516 16.8 
...
**PARAMETER† TEST CONDITIONS MIN TYP MAX UNIT**
f(SCL) Clock frequency (I^2 C only) 0 400 kHz
t(BUF) Bus free time between start and stop condition 1.3 μs
t(HDSTA) Hold time after (repeated) start condition. Afterthis period, the first clock is generated. 0.6 μs
...
##### When a start condition is detected on the I^2 C bus, the device transitions to the Idle state where it checks the
...
```
```
Clear ADC CDATAH(r 0x15), CDATA(r 0x14)
Red ADC
Green ADC
Blue ADC
```
...
```
PROGRAMMABLE
PARAMETER PROGRAMMED VALUE DURATION
```
```
TYPICAL
CURRENT
...
Wait
WTIME 0xEE
Wait WLONG 0 43 2 ms43.2 ms 0 065 mA0.065 mA
...
##### Interface and control are accomplished through an I^2 C serial compatible interface (standard or fast mode) to

##### a set of registers that provide access to device control functions and output data. The devices support the 7-bit

##### I^2 C addressing protocol.

##### The I^2 C standard provides for three types of bus transaction: read, write, and a combined protocol (Figure 10).
...
##### The I^2 C bus protocol was developed by Philips (now NXP). For a complete description of the I^2 C protocol, please

##### review the NXP I^2 C design specification at http://www.i2c−bus.org/references/.
...
−− COMMAND W Specifies register address 0x
0x00 ENABLE R/W Enables states and interrupts 0x
0x01 ATIME R/W RGBC time 0xFF
0x03 WTIME R/W Wait time 0xFF
0x04 AILTL R/W Clear interrupt low threshold low byte 0x
0x05 AILTH R/W Clear interrupt low threshold high byte 0x
0x06 AIHTL R/W Clear interrupt high threshold low byte 0x
0x07 AIHTH R/W Clear interrupt high threshold high byte 0x
0x0C PERS R/W Interrupt persistence filter 0x
0x0D CONFIG R/W Configuration 0x
0x0F CONTROL R/W Control 0x
0x12 ID R Device ID ID
0x13 STATUS R Device status 0x
0x14 CDATAL R Clear data low byte 0x
0x15 CDATAH R Clear data high byte 0x
0x16 RDATAL R Red data low byte 0x
0x17 RDATAH R Red data high byte 0x
0x18 GDATAL R Green data low byte 0x
0x19 GDATAH R Green data high byte 0x
0x1A BDATAL R Blue data low byte 0x
0x1B BDATAH R Blue data high byte 0x
```
##### The mechanics of accessing a specific register depends on the specific protocol used. See the section on I^2 C
...
```
```
ENABLE Reserved AIEN WEN Reserved AEN Address0x
```
```
FIELD BITS DESCRIPTION
Reserved 7:5 Reserved. Write as 0.
AIEN 4 RGBC interrupt enable. When asserted, permits RGBC interrupts to be generated.
WEN 3 Wait enable. This bit activates the wait feature. Writing a 1 activates the wait timer. Writing a 0 disables thewait timer.
Reserved 2 Reserved. Write as 0.
AEN 1 RGBC enable. This bit actives the two-channel ADC. Writing a 1 activates the RGBC. Writing a 0 disablesthe RGBC.
```
```
...
```
75426 310
CONFIG Reserved WLONG Reserved Address0x0D
```
```
FIELD BITS DESCRIPTION
Reserved 7:2 Reserved. Write as 0.
```
##### WLONG 1 Wait Long. When asserted, the wait cycles are increased by a factor 12WTIME register. × from that programmed in the
...
```
75426 310
CONTROL Reserved AGAIN Address0x0F
```
```
FIELD BITS DESCRIPTION
...
```
```
0x44 = TCS34721 and TCS
ID 7:0 Part number identification 0x4D = TCS34723 and TCS
```
...
```
3102
STATUS Reserved AINT Reserved Address0x
```
```
...
##### I^2 C transaction should be used with a read word protocol bit set in the command register. With this operation,
...
Time above 217°C (T1) t 1 Max 60 sec
Time above 230°C (T2) t 2 Max 50 sec
Time above Tpeak −10°C (T3) t 3 Max 10 sec
