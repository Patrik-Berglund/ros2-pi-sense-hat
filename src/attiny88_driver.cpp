...
bool ATTiny88Driver::initFrameSync() {
  ...
  // Request line for rising edge events
...
void ATTiny88Driver::waitForFrameSync() {
  ...
  // Wait up to 20ms for rising edge event
  struct timespec timeout = {0, 20000000}; // 20ms
...
void ATTiny88Driver::setAll(const uint8_t* rgb_data, size_t length) {
  ...
  // Wait for frame sync before updating
  waitForFrameSync();
  ...
  // Single I2C write for entire frame
